...
const EXPLICIT_CURRENCY_ALIASES: Record<string, CurrencyCode> = {
  "$": "USD", "US$": "USD", "CA$": "CAD", "CN¥": "CNY", "RMB": "CNY",
  "HK$": "HKD", "€": "EUR", "£": "GBP", "¥": "CNY", "￥": "CNY", "JP¥": "JPY",
};
...
function createCurrencySymbolAliases(): Record<string, CurrencyCode> {
  ...
  return symbolEntries.reduce<Record<string, CurrencyCode>>((aliases, [symbol, currency]) => {
    if (symbol && symbolCounts[symbol] === 1) aliases[symbol] = currency;
    return aliases;
  }, {});
}
...
export function calculateTotalRemainingValueCNY(
  ...
  return nodes.reduce((sum, node) => {
    if (excludeFreeTags && node.tags?.includes("白嫖中")) return sum;
...
export function calculateValueCNY(node: NodeData, exchangeRates: ExchangeRates): number {
...
export function calculateRemainingValueCNY(
  ...
  if (diffMs > 0 && billingCycleMs > 0) return priceCNY * (diffMs / billingCycleMs);
  return 0;
}
...
export function formatFinanceAmount(amount: number, currency: CurrencyCode): {
  ...
  return { currency, symbol: CURRENCY_SYMBOLS[currency], value };
}
...
export async function getDailyExchangeRates(): Promise<{
  ...
  if (cached && cached.date === today) return { rates: cached.rates, source: "cache" };
...
function writeCachedExchangeRates(rates: ExchangeRates, date: string): void {
  const cache: ExchangeRatesCache = { base: "CNY", date, fetchedAt: Date.now(), rates };
